...
export function ProductCard({ product, priority }: { product: Product; priority?: boolean }) {
  ...
  return (
    <article className="flex h-full flex-col rounded-3xl border border-stone bg-paper p-5">
      <Link href={`/products/${product.slug}`} className="block" tabIndex={-1} aria-hidden="true">
        <ImageFrame src={src} alt={product.heroImageAlt ?? product.name} priority={priority} className="bg-white" label="Image to follow" />
      </Link>
      <div className="flex flex-1 flex-col px-2 pt-5 pb-2">
        <p className="text-(length:--step--1) font-medium text-accent">{manufacturerName(product.manufacturer)}</p>
        <h3 className="mt-1">
          <Link href={`/products/${product.slug}`} className="hover:underline underline-offset-4">{product.name}</Link>
        </h3>
        <p className="mt-2 text-slate">{product.shortDescription}</p>
