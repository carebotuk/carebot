...
type Props = {
  children: ReactNode;
  band?: "paper" | "mist" | "ink" | "sage" | "green";
  id?: string;
  className?: string;
  narrow?: boolean;
...
export function Section({ children, band = "paper", id, className = "", ariaLabelledBy }: Props) {
  const bg =
    band === "green" ? "bg-accent text-paper" : band === "sage" ? "bg-sage" : band === "mist" ? "bg-mist" : band === "ink" ? "bg-ink text-paper" : "bg-paper";
  return (
...
export function SectionHeading({
  ...
  return (
    <div className="mb-10 max-w-3xl sm:mb-14">
      {kicker ? <p className="mb-3 text-(length:--step--1) font-medium text-accent">{kicker}</p> : null}
      <h2 id={id}>{title}</h2>
